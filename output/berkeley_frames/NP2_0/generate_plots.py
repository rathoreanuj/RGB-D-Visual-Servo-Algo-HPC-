...
frame_output = Path(r"output\berkeley_frames\NP2_0")
results_file = frame_output / 'data' / 'benchmark_results.json'

with open(results_file, 'r') as f:
    data = json.load(f)

frame_data = data['NP2_0']
threads = [1, 2, 4, 8]
...
fig.suptitle(f'Histogram Equalization - Frame: NP2_0', fontsize=18, fontweight='bold', y=0.995)
plt.tight_layout()
...
fig.suptitle(f'Performance Analysis - Frame: NP2_0', fontsize=18, fontweight='bold', y=0.995)
plt.tight_layout()
...
fig.suptitle(f'Convergence Trajectories - Frame: NP2_0', fontsize=18, fontweight='bold')
plt.tight_layout()
plt.savefig(frame_output / 'images' / 'convergence_comparison.png', dpi=300, bbox_inches='tight')
plt.close()

print(f'Generated plots for NP2_0')
