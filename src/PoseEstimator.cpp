...
Eigen::Matrix3d PoseEstimator::computeRotationMatrix(double alpha, double beta, double gamma) {
    ...
    // where Rx, Ry, Rz are rotation matrices around X, Y, Z axes
...
std::vector<std::pair<cv::Point2d, cv::Point2d>> PoseEstimator::projectModel(
    ...
    // Get transformation matrix (Equation 14)
    Eigen::Matrix4d T = computeTransformationMatrix(pose);
    
    // Transform all vertices from model coordinates to camera coordinates
    std::vector<Eigen::Vector3d> transformedVertices;
    transformedVertices.reserve(model.vertices.size());
    
    // ADVANCED OPENMP: Parallel vertex transformation with SIMD
    ...
    // Project edges to 2D image plane
    // ADVANCED OPENMP: Thread-local accumulation to avoid critical sections
    ...
    #pragma omp parallel
    {
        ...
        for (int e = 0; e < static_cast<int>(model.edges.size()); e++) {
            ...
            if (v1_idx >= 0 && v1_idx < static_cast<int>(transformedVertices.size()) &&
                ...
                // Only project edges that are in front of camera (positive Z)
    ...
    // Merge thread-local results efficiently
    size_t totalSize = 0;
